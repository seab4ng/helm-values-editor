const HelmLib = (function () {
  ...
  function flatten(obj, prefix, out) {
    if (obj === null || obj === undefined) {
      out.push({ path: prefix, val: null, type: 'null' });
      return;
    }
    if (Array.isArray(obj)) {
      if (!obj.length) {
        out.push({ path: prefix, val: '[]', type: 'arr' });
        return;
      }
  ...
  function buildChartTree(fileMap, parseYaml, rootFallback) {
    ...
    const chartYamls = Object.keys(fileMap).filter(
      p => p.toLowerCase().endsWith('/chart.yaml') || p.toLowerCase() === 'chart.yaml'
    );
    ...
    function discoverChart(chartYamlPath, parent, nameHint) {
      ...
      if (valuesContent) {
        try {
          tempData[name] = parseYaml(valuesContent) || {};
  ...
  function cleanStaleBracketKeys(obj) {
    ...
    if (Array.isArray(obj)) {
      obj.forEach(cleanStaleBracketKeys);
      return;
    }
  ...
  function cleanDottedKeyCollisions(obj) {
    if (!obj || typeof obj !== 'object') return;
    if (Array.isArray(obj)) {
      obj.forEach(cleanDottedKeyCollisions);
      return;
    }
    for (const key of Object.keys(obj)) {
      if (key.includes('.')) {
